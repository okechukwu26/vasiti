...
import {addBooking, AssignBus, GetBookingWithVehicle,updateBooking, vehicleStatus,InTransit,bookingStatus,passengerStatus} from './bookingInterface'
         ...
         const payment =  await this.verifyPayment(trip.price,bookingData.referenceId,bookingData.numberOfTravellers, user)
           ...
           for (const passenger of bookingData.passenger){   
               ...
               const bookingModel = Bookings.create(bookingData)
                ...
                const ReturnTrip = await Trips.findOneOrFail({id: bookingData.returnTripId})
                .catch(() =>{
                ...
                if(!available){
                 ...
                 const Return = dayjs(bookingData.returnDate).format('dddd').toLowerCase()
                ...
                const amount= Number(bookingData.amount/bookingData.numberOfTravellers)
    ...
    throw new AppError(`Please select ${passengers} seats. Only ${seats.length} selected!`, seats)
        ...
        try {
            for(const newSeat of seat){
                ...
                const book = await Bookings.findOne({where:[{
                    seat:newSeat,
...
return seats
    ...
    public assignus = async(bookingData:AssignBus, user:Users) =>{
      ...
      const route = await Routes.findOneOrFail({where:[{
          id:trip.route.id
      }], relations:["terminal"]})
      .catch(() =>{
      ...
      const type = await VehicleType.findOneOrFail({where:[{
           id:trip.type.id
       }], relations:['feature']})
       .catch(() =>{
    ...
    if(!vehicle){
        throw new AppError('vehicle is already in transit or already assigned to a trip')
    }
       ...
       const booking =  await Bookings.find({where:[{
        ...
        const booked = await Bookings.find({where:[{
            vehicle:vehicle.id
        }]})
        ...
        if(bookingData.service == "book_a_seat"){
            if(booking.length > type.seatNumber){
                throw new AppError('number of passengers is greater than vehicle seat number')
            }
            if(booking.length === 0){
            ...
            booking.forEach(async item =>{
                if(item.bookingStatus == BookingStatus.CHECK_IN_AND_APPROVED ){
                    ...
                    throw new AppError('passenger has to be checked in before assigning a vehicle to them')
                }
             })
    ...
    relations:["terminal"]})
       ...
       const booking =  await Bookings.findOne({where:[{
    ...
    if(booking === undefined){
      throw new AppError('no booking has been done on this trip and on this day')         
    }
   

        const vehicle = await Vehicles.findOneOrFail({where:[{id:transitData.vehicleId, vehicleStatus:VehicleStatus.ASSIGNED}]})
        .catch(() =>{
          ...
          await vehicle.save()
          const terminal = await Terminals.findOneOrFail({where:[{id:user.Terminal}], relations:["state",'lga']})
...
public GetBookingWithVehicles = async(bookingData:GetBookingWithVehicle, user:Users) =>{
    ...
    if(!user.priviledges.includes('manager')){
       ...
       const booking =  await Bookings.find({where:[{
        ...
        for (let profile of booking){
            ...
            const prof = await Passengers.find({where:[{id: profile.passengerId.id}],relations:['profile']})
    ...
    public vehicleArrived = async(status:vehicleStatus, user:Users) =>{ 
        ...
        const route = await Routes.findOneOrFail({where:[{id:status.routeId}], relations:["terminal"]})
        .catch(() =>{
            throw new AppError('invalid route selected')
        })
        ...
        let vehicle = await Vehicles.findOne({where:[{
            id:status.id
